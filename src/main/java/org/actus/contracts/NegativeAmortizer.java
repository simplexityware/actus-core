...
public final class NegativeAmortizer {
    ...
    public static ArrayList<ContractEvent> events(LocalDateTime analysisTime,
                                                  ...
                                                  ContractModelProvider model,
                                                  RiskFactorModelProvider riskFactorModel) throws AttributeConversionException {
        return NegativeAmortizer.events(Stream.of(analysisTime,analysisTime.plus(period)).collect(Collectors.toSet()),model,riskFactorModel);
    }
    ...
    public static ArrayList<ContractEvent> transactions(Set<LocalDateTime> analysisTimes,
                                                        ContractModelProvider model,
                                                        RiskFactorModelProvider riskFactorModel) throws AttributeConversionException {
        return NegativeAmortizer.events(analysisTimes,model,riskFactorModel).stream().filter(ev->StringUtils.TransactionalEvents.contains(ev.type())).collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public static ArrayList<ContractEvent> transactions(LocalDateTime analysisTime,
                                                        ...
                                                        ContractModelProvider model,
                                                        RiskFactorModelProvider riskFactorModel) throws AttributeConversionException {
        return NegativeAmortizer.transactions(Stream.of(analysisTime,analysisTime.plus(period)).collect(Collectors.toSet()),model,riskFactorModel);
    }
    ...
    public static ArrayList<ContractEvent> payoff(Set<LocalDateTime> analysisTimes,
                                                  ContractModelProvider model) throws AttributeConversionException {
        return NegativeAmortizer.lifecycle(analysisTimes,model).stream().filter(ev->StringUtils.TransactionalEvents.contains(ev.type())).collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public static ArrayList<ContractEvent> events(LocalDateTime analysisTime,
                                                  Period period,
                                                  ContractModelProvider model) throws AttributeConversionException {
        return NegativeAmortizer.events(Stream.of(analysisTime,analysisTime.plus(period)).collect(Collectors.toSet()),model);
    }

    // compute non-contingent transactions in time window
    public static ArrayList<ContractEvent> transactions(Set<LocalDateTime> analysisTimes,
                                                        ContractModelProvider model) throws AttributeConversionException {
        return NegativeAmortizer.events(analysisTimes,model).stream().filter(ev->StringUtils.TransactionalEvents.contains(ev.type())).collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public static ArrayList<ContractEvent> transactions(LocalDateTime analysisTime,
                                                        Period period,
                                                        ContractModelProvider model) throws AttributeConversionException {
        return NegativeAmortizer.transactions(Stream.of(analysisTime,analysisTime.plus(period)).collect(Collectors.toSet()),model);
    }
